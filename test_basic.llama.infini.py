...
#     device = "cuda"  # set GPU device using CUDA_VISIBLE_DEVICES
# else:
#     device = "cpu"
...
outputs = model(**encoded.to(model.device))  # position_ids=position_ids)
print("Short Text Loss")
print(outputs.loss)
outputs.loss.backward()  # Test the backward pass

outputs = model(**long_encoded.to(model.device))  # position_ids=position_ids)
